...
import { FaPlane, FaFilter, FaClock, FaExchangeAlt, FaSuitcase, FaUtensils, FaWifi, FaChevronDown, FaChevronUp, FaPhone } from "react-icons/fa";
import { internalDuffelApi } from "@/lib/duffel-client";
import LoadingAnimation from "@/components/ui/LoadingAnimation";

// Define types for Sky-Scrapper flight data
...
export default function FlightSearchResults() {
  const searchParams = useSearchParams();  const [isLoading, setIsLoading] = useState(true);
  ...
  useEffect(() => {
    ...
    console.log('Search params:', { from, to, departureDate, returnDate, travelers });
  ...
  const fetchFlightsFromApi = async (
    ...
    try {
      ...
      console.log("Calling Duffel API for flights:", {
        origin,
        destination,
        departureDate,
        returnDate,
      ...
      try {
        ...
        if (response && response.offers) {
          ...
          if (processedFlights.length > 0) {
            ...
            setError("No flights found for your search criteria. Duffel Test Environment might have limited routes. Try major hubs like LHR, JFK, or DEL.");
          }
        } else {
          setError("No flight offers returned from the server.");
        }
      } catch (apiErr: any) {
        console.error("Duffel API Error:", apiErr);
        
        if (retryCount < 2) {
          console.log(`Retrying Duffel call (${retryCount + 1}/2)...`);
          setTimeout(() => {
            fetchFlightsFromApi(origin, destination, departureDate, returnDate, travelers, retryCount + 1);
          }, 2000);
          return;
        }

        setError(apiErr.message || "Failed to fetch flights from Duffel. Please ensure your Test Token is valid.");
      }
    } catch (err: any) {
                ...
                <>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="space-y-2">
                      {[0, 1, 2].map((stop) => (
                        <label key={stop} className="flex items-center cursor-pointer hover:bg-gray-50 p-2 rounded">
                          <input
                            type="checkbox"
            ...
            {isLoading ? (
              ...
              <div className="bg-white rounded-xl shadow-md p-8 text-center">
                ...
                <p className="text-gray-600 mb-4">
                  {typeof error === 'string' ? error : "An unexpected error occurred with the search parameters."}
                </p>
                <p className="text-gray-600 mb-6">
                  Please try adjusting your search or try again later.
                </p>
                <button 
              ...
              <div className="space-y-4">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between">
                          ...
                          <div className="flex items-center mb-4 md:mb-0">
                            <div className="w-12 h-12 bg-gray-100 rounded-full flex items-center justify-center mr-4 overflow-hidden">
                              {mainCarrier.logoUrl ? (
                                <img src={mainCarrier.logoUrl} alt={mainCarrier.name} className="w-full h-full object-contain" />
                              ) : (
                                <span className="text-sm font-bold">{mainCarrier.name.substring(0, 2).toUpperCase()}</span>
                              )}
                            </div>
                            <div>
                          ...
                          <div className="flex-grow flex flex-col md:flex-row justify-between items-center mb-4 md:mb-0">
                            ...
                            <div className="flex flex-col items-center mx-4 my-2 md:my-0">
                              <p className="text-xs text-gray-500 font-medium">{formatDuration(outboundLeg.durationInMinutes)}</p>
                            ...
                            <div className="text-center md:text-right">
                              ...
                              <p className="text-sm text-gray-600 font-semibold">{outboundLeg.destination.displayCode}</p>
                                  ...
                                  {flight.legs.map((leg, lIdx) => (
                                    <div key={leg.id} className="bg-gray-50 p-3 rounded-lg">
                                      ...
                                      <div className="text-sm text-gray-600 space-y-1">
                                        <p><span className="font-medium">Departure:</span> {leg.origin.name} ({leg.origin.displayCode})</p>
                                        <p><span className="font-medium">Arrival:</span> {leg.destination.name} ({leg.destination.displayCode})</p>
                                        <p><span className="font-medium">Total Duration:</span> {formatDuration(leg.durationInMinutes)}</p>
                                        <div className="mt-2 text-xs">
                                          <p className="font-medium text-gray-500">Airlines:</p>
                                          <div className="flex flex-wrap gap-2 mt-1">
                                            {leg.carriers.marketing.map((c, cIdx) => (
                                              <span key={cIdx} className="bg-white px-2 py-1 rounded border border-gray-200">
                                                {c.name}
                                              </span>
                                            ))}
                                          </div>
                              ...
                              <div>
                                ...
                                <div className="space-y-4">
                                  ...
                                  <div className="bg-green-50 p-3 rounded-lg border border-green-100">
                                    ...
                                    <p className="text-xs text-green-700 mb-2">Call for exclusive deals and personalized support</p>
                                    <a href="tel:+18001234567" className="inline-flex items-center text-sm font-bold text-green-700 hover:text-green-800">
                ...
                <div className="mt-8 p-6 bg-white rounded-xl shadow-md border border-primary-100">
                  <h3 className="text-xl font-bold mb-4 text-gray-800">Need Help Finding the Perfect Flight?</h3>
                  <p className="text-gray-600 mb-4">
                    Our travel experts can provide personalized recommendations and often find exclusive deals not available online.
                    Get the best value for your trip by speaking with our specialists.
                  </p>
                  
                  <div className="grid md:grid-cols-2 gap-4 mb-6">
                    <div className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                      <h4 className="font-medium mb-2">Premium Support</h4>
                      <p className="text-sm text-gray-600 mb-3">Get assistance with complex itineraries and special requests</p>
                      <a href="tel:+18001234567" className="text-primary-600 font-medium flex items-center">
                        <FaPhone className="mr-1" /> Call for Premium Support
                      </a>
                    </div>
                    
                    <div className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                      <h4 className="font-medium mb-2">Exclusive Deals</h4>
                      <p className="text-sm text-gray-600 mb-3">Access unpublished fares and special negotiated rates</p>
                      <a href="tel:+18001234567" className="text-primary-600 font-medium flex items-center">
                        <FaPhone className="mr-1" /> Call for Special Deals
                      </a>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg flex items-center justify-between">
                    <div>
                      <p className="font-medium text-gray-800">Ready to book your flight?</p>
                      <p className="text-sm text-gray-600">Our experts are available 24/7</p>
                    </div>
                    <a 
                      href="tel:+18001234567" 
                      className="bg-primary-600 text-white hover:bg-primary-700 transition-colors px-6 py-2 rounded-full font-medium flex items-center"
                    >
